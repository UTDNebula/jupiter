...
const OrgDirectoryCards: React.FC<Props> = ({ club }) => {
  ...
  const onClick = () => {
    router.push(`/directory/${club.name}`);
  };

  return (
    <div className="bg-white rounded-lg shadow-lg max-w-sm h-144 flex flex-col">
      <div className="relative h-64">
        ...
        <button className="absolute top-2 right-2 bg-white text-slate-800 font-bold py-1 px-2 rounded-lg">
          {'<3'}
        </button>
      </div>
      <div className="p-4">
      ...
      <div className="just flex p-4 justify-end">
